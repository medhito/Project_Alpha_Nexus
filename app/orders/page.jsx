...
export default function BuyerOrdersPage() {
    ...
    const [orders, setOrders] = useState([
        {
            ...
            items: [
                { title: 'Handmade Minimalist Ceramic Mug', qty: 2, price: 24.00 }
            ],
        ...
        const timer = setInterval(() => {
            setOrders(currentOrders =>
                currentOrders.map(order => ({
                    ...order,
                    cancelTimeRemaining: order.cancelTimeRemaining > 0 ? order.cancelTimeRemaining - 1 : 0
                }))
            );
        }, 1000);
    ...
    const handleCancelOrder = (orderId) => {
        setOrders(currentOrders =>
            currentOrders.map(order =>
                order.id === orderId ? { ...order, status: 'cancelled_by_buyer' } : order
            )
        );
    };
            ...
            <div className={styles.ordersList}>
                        ...
                        <div className={styles.orderActions}>
                            ...
                            {order.status === 'pending' && order.cancelTimeRemaining > 0 && (
                                <div className={styles.cancelWrapper}>
                                    <span className={styles.timerText}>
                                        Cancel window ends in: {formatTime(order.cancelTimeRemaining)}
                                    </span>
                                    <button
                            ...
                            {order.status === 'cancelled_by_seller' && (
                                <div className={styles.alertBox}>
                                    <AlertCircle size={16} />
                                    <span>The seller cancelled this order (Out of stock / Cannot fulfill).</span>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                {orders.length === 0 && (
                    <div className={styles.emptyState}>
                        ...
                        <Link href="/" className={styles.browseButton}>Browse Local Market</Link>
                    </div>
                )}
            </div>
        </div>
